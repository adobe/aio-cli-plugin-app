...
module.exports = () => {
  ...
  const legacyAppConfig = loadLegacyAppConfig(commonConfig)
  userConfig.application = mergeAppConfig(userConfig.application, legacyAppConfig)

  // load the full standalone application configuration
  const appAndExtConfigs = loadAppAndExtConfigs(userConfig, commonConfig)

  return {
    configs: appAndExtConfigs,
 ...
 */
function mergeAppConfig (appConfig, legacyAppConfig) {
 ...
 * @param commonConfig
 */
function loadAppAndExtConfigs (userConfig, commonConfig) {
  const allConfigs = loadExtensionConfigs(userConfig, commonConfig)
  if (userConfig.application) {
    allConfigs.application = loadSingleConfig('application', userConfig.application, commonConfig)
  }
  return allConfigs
}
 ...
 * @param commonConfig
 */
function loadExtensionConfigs (userConfig, commonConfig) {
  const configs = {}
...
function loadSingleConfig (configTag, singleUserConfig, commonConfig) {
  ...
  config.manifest = { src: 'manifest.yml' }
  config.manifest.full = singleUserConfig.runtimeManifest
