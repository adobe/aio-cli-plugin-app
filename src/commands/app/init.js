...
class InitCommand extends TemplatesCommand {
  ...
  async selectOrCreateConsoleProject (consoleCLI, org, flags) {
    ...
    if (flags.yes) {
      ...
      // No --project supplied. Auto-generate a unique name of the form "generatedAppN"
      // where N is the lowest positive integer not already used by an existing project.
      // This mimics sequential behaviour (generatedApp1, generatedApp2, ...) and fills
      // gaps left by deleted projects (e.g. if generatedApp2 was deleted, it is reused
      // before generatedApp4 is attempted).
      const existingNames = new Set(projects.map(p => p.name))
      let suffix = 1
      while (existingNames.has(`generatedApp${suffix}`)) {
        suffix++
      }

      const generatedName = `generatedApp${suffix}`
      const generatedTitle = `AB Project ${suffix}`
      const generatedDescription = `App Builder Project ${suffix} - generated by an agent`
