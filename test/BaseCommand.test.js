...
describe('getFullConfig', () => {
  ...
  test('with options', async () => {
    ...
    expect(config).toEqual({ a: 'hello' })
    expect(mockConfigLoader.load).toHaveBeenCalledWith({ someOptions: {} })
  })
})
...
describe('getAppExtConfigs', () => {
  ...
  test('with options', async () => {
    ...
    expect(mockConfigLoader.load).toHaveBeenCalledWith({ some: 'options' })
  })
  ...
  test('-e exc -e notexists', async () => {
    ...
    mockConfigLoader.load.mockResolvedValue(config)
    const cmd = new TheCommand()
    await expect(cmd.getAppExtConfigs({ extension: ['exc', 'notexists'] }))
  ...
  test('-e dx (matches more than one)', async () => {
    ...
    mockConfigLoader.load.mockResolvedValue(config)
    const cmd = new TheCommand()
    await expect(cmd.getAppExtConfigs({ extension: ['dx'] }))
