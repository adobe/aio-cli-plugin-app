...
const buildExtensionPointPayloadWoMetadata = (deployConfigs) => {
  const payload = {
    endpoints: {}
  }

  // Add excshell extension point if present
  if (deployConfigs['dx/excshell/1']) {
    payload.endpoints['dx/excshell/1'] = {
      view: [{
        name: 'test-app',
        url: 'https://adobeio-static.net/test-app/index.html',
        metadata: {
          name: 'test-app',
  ...
  // Add asset compute worker extension point if present
  if (deployConfigs['dx/asset-compute/worker/1']) {
    payload.endpoints['dx/asset-compute/worker/1'] = {
      workerProcess: []
    }
  }
...
const buildExcShellViewExtensionMetadata = async (libConsoleCLI, aioConfig) => {
  return {
    name: 'test-app',
    description: 'Test App',
    version: '1.0.0'
